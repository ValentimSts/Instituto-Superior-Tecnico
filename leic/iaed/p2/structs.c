...
tlink* create_node() {
	tlink* node = (struct tree_node*) malloc(sizeof(struct tree_node));

	node->path = NULL;
	node->value = NULL;
	node->n_children = 0;
	node->len = BRANCH_SIZE;
	
	node->children = (struct tree_node**) malloc(sizeof(struct tree_node*) * BRANCH_SIZE);
...
/**
 * Creates a new tree node containing the given path and value,
 * and returns it. 
 */
